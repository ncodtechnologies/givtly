...
class ListFrm extends Component {
  ...
  componentDidMount = () => {
    const id_sub_category = this.props.navigation.state.params.id_sub_category;
    this.props.getPostList(id_sub_category, "");
  };
  constructor(props){
    super(props);
    this.state = {
  ...
  render() {
    return (
      <View >
        ...
        {(this.state.showSearch)?(
          <View style={{ flexDirection:"row", marginTop:5,  padding: 5, paddingHorizontal: 10 , borderColor: '#E7E2E2',borderRadius: 20, borderWidth: 1,}} >
              ...
              />
              <TouchableOpacity style={{ width: Dimensions.get('window').width*0.15, height: 40 }} onPress={()=>this.props.getPostList(this.state.id_sub_category, this.state.filter)} >
          ...
          <View style={{ flexDirection:"row", marginTop:5,  padding: 5, paddingHorizontal: 10 , borderColor: '#E7E2E2',borderRadius: 20, borderWidth: 1,}} >
              <Picker
                ...
                onValueChange={(itemValue, itemIndex) => {
                            ...
                            this.props.getPostList(this.state.id_sub_category, this.state.filter);
                  }
                }>
     ...
     <View>
      ...
      renderRow={data =>
        <TouchableOpacity onPress={() => this.props.navigation.navigate("PostDt", { post : data })} >
          <View style={styles.box}><GImageDescription id_post={data._id} place={data.place} price={data.price} name={data.title} date={data.date}  /></View><GBorder/></TouchableOpacity>
      }
    />
    </View>
    )
...
const mapDispatchToProps = dispatch => ({
  getPostList: (id_sub_category, filter) => dispatch(getPostList(id_sub_category, filter)),
});
